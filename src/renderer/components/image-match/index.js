        ...
        function _choponent_size_fn(e) {
          ...
          if (el._type === 'auto') {
            if (
              ...
              el._childrenWidth / el._childrenHeight
            ) {
              const ty =
                ((el._childrenHeight -
                  (parentHeight * el._childrenWidth) / parentWidth) /
                  2 /
                  (el._childrenWidth / parentWidth)) *
                -1
              children.style.cssText = `width:${parentWidth}px;height:auto;transform: translateY(${ty}px);`
            } else {
              const tx =
                ((el._childrenWidth -
                  (parentWidth * el._childrenHeight) / parentHeight) /
                  2 /
                  (el._childrenHeight / parentHeight)) *
                -1
              children.style.cssText = `width:auto; height:${parentHeight}px;transform: translateX(${tx}px);`
            }
          }
