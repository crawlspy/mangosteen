<template>
  ...
  >
    <div class="header">
      ...
      <div
        class="header-tag"
        v-if="currentImageSource.search && searchKeyList.length > 0"
      >
        <div
        ...
        >
          ...
          <span
            class="header-tag-item-del"
            @click.stop="searchKeyListDelete(tag)"
            >x</span
          >
        </div>
      </div>
      ...
      <div
        class="header-tag category-tag"
        v-if="currentImageSource.category && categoryList.length > 0"
      >
        <div
          :class="['header-tag-item', tag.id === category ? 'active' : '']"
    ...
    >
      <div class="content-main" ref="content_main" v-if="images.length > 0">
        <div
          ...
          @mousemove.stop="
            ;(currentMouseOverIndex = index), (setterShow = false)
          "
        ...
        >
          ...
          <div
            ...
            @click.stop="setWallpaper(img, index)"
          >
...
export default {
  ...
  computed: mapState({
    ...
    currentImageSource() {
      return imageSourceType.find(item => item.value === this.imageSource)
    }
  }),
  ...
  methods: {
    ...
    eventInit() {
      ...
      this.$ipcRenderer.on('intervalTime', (event, arg) => {
        ...
        const statisticTimeFlag = this.$localStorage.getStore(
          'statisticTimeFlag'
        )
        if (!statisticTimeFlag) {
          this.$localStorage.setStore('statisticTimeFlag', nowDate)
        }
        const timingWipeDataFlag = this.$localStorage.getStore(
          'timingWipeDataFlag'
        )
    ...
    domContentMainMatch() {
      this.$nextTick(() => {
        if (this.$refs.content_main) {
          this.$refs.content_main.style.paddingTop = `${
        ...
        if (this.$localStorage.getStore('lastUpdataTime')) {
          ...
          if (!isNaN(parseInt(userConfig.updataTime, 10))) {
            ...
            if (Math.abs(currentTime - time) > updataTime) {
              const index = this.images[this.currentWallpaperIndex]
    ...
    searchKeyFn() {
      ...
      if (
        !this.searchKeyList.includes(this.searchKey) &&
        this.searchKey !== ''
      ) {
        this.domContentMainMatch()
