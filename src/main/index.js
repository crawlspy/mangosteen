  ...
  appTray.on('click', (event, bounds, position) => {
    ...
    function setMainWinPosition(win, trayBounds) {
      try {
        ...
        const parallelType =
          cursorPosition.x <
          currentScreen.bounds.x + currentScreen.workAreaSize.width / 2
            ? 'left'
            : 'right'
        const verticaType =
          cursorPosition.y <
          currentScreen.bounds.y + currentScreen.workAreaSize.height / 2
            ? 'top'
            : 'bottom'
        ...
        if (currentScreen.workAreaSize.height < currentScreen.size.height) {
          ...
          trayPositionSize =
            currentScreen.size.height - currentScreen.workAreaSize.height
        } else if (
          currentScreen.workAreaSize.width < currentScreen.size.width
        ) {
          trayPositionType = parallelType === 'left' ? 'left' : 'right'
          trayPositionSize =
            currentScreen.size.width - currentScreen.workAreaSize.width
        }
        let winPositionX = 0
        let winPositionY = 0
        if (trayPositionType === 'top') {
          winPositionX = Math.max(
            Math.min(
              currentScreen.bounds.width + currentScreen.bounds.x - winWidth,
              cursorPosition.x - winWidth / 2
            ),
          ...
          winPositionY = Math.max(
            Math.min(
              currentScreen.bounds.height + currentScreen.bounds.y - winHeight,
              cursorPosition.y - winHeight / 2
            ),
            currentScreen.bounds.y
          )
...
function ipcMainInit() {
  ...
  ipcMain.on('dataWallpaper', (event, arg) => {
    downloadPic(arg.downloadUrl, mainWindow)
      .then(result => {
